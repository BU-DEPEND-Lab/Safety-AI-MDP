...
for i in range(coords[0]):
	...
	for j in range(6):
		feature_states[-1].append([int(S/coords[0] * j/5), i])
...
while real == 'Y' or real == 'y':
    ...
    # thing once and be done with it.
    		if sum(streak) < 70 and episodes > 100:
			using -= episodes
        		episodes = 0
...
start = grids.states[int(start_s/grids.x_max)][int(start_s%grids.x_max)]
...
if real == 'Y' or real == 'y':
	safety_spec = raw_input("Safety spec?")
	try:
	...
	_, theta, policy, _  = update.expert_synthesize(grids, demo_mu, agent, [start], steps = steps, epsilon = 1e-5, iteration = 30, gamma = gamma, start_theta = theta, MC = True, unsafe =unsafe, safety = safety)
	#grids.w_features(theta)
	#learnt_policy, value = update.optimal_value(grids, agent, steps = steps, epsilon= 1e-5, gamma = gamma, toolbox = False, starts = [start])
