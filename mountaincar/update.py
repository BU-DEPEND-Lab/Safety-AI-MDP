...
def expert_synthesize1(grids, expert, agent, starts, steps, epsilon=1e-6, iteration=100, gamma=0.5, start_theta= None, MC = False, unsafe = None, safety = 0.0001):
	...
	policies = []	
	grids.w_features(start_theta)
	start_policy, start_value = optimal_value(grids, agent, steps = steps, epsilon=epsilon, gamma = gamma)
	policies = policies + [start_policy]
	return start_theta, start_policy, start_value, None
	if MC is False:
		start_mu, MU = optimal_feature(grids, starts, steps, start_policy, epsilon = epsilon, gamma=gamma)
	else:
		exp_u_G, p_G, exp_u_B, p_B, _ = sample_feature(grids, agent, starts, steps, start_policy, epochs= 5000, epsilon = epsilon, gamma=gamma, bounds = np.zeros([len(starts)]), unsafe = unsafe)
		start_mu = np.sum(np.reshape(exp_u_G, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]) * np.reshape(p_G, [grids.y_max*grids.x_max, 1]) +   np.reshape(exp_u_B, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]) * np.reshape(p_B, [grids.y_max*grids.x_max, 1]), 0) 
	...
	while True:
		...
		if p_B_sum is not None and p_B_sum > safety:
			...
			#p_B_sum = np.sum(np.reshape(p_B, [grids.y_max*grids.x_max]))/(len(starts))
			print "Policy unsafe rate ", p_B_sum
			while p_B_sum < safety and np.linalg.norm(new_mu - mus[-1], ord = 2)> epsilon:
				print "Same k, found different safe policy"
				if MC is False:
					new_mu, MU = optimal_feature(grids, starts, steps, new_policy, epsilon = epsilon, gamma=gamma)
				else:
...
def expert_synthesize2(grids, expert, agent, starts, steps, epsilon=1e-6, iteration=100, gamma=0.5, start_theta= None, MC = False, unsafe = None, safety = 0.0001):
	...
	if p_B_sum <= safety and p_B_sum is not None:
		...
		return None
	while p_B_sum > safety:
		new_mu_B =  counterexample(grids, MU, gamma, safety, epsilon, steps)
		#new_mu_B = np.sum(np.reshape(exp_u_B, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]), 0) 
		if new_mu_B is not None:
			mu_Bs = mu_Bs + [new_mu_B]
			print "Add counterexample features ", new_mu_B	
	...
	while p_B_sum > safety:
		...
		if new_mu_B is not None:
			if np.linalg.norm(new_mu_B - mu_Bs[-1], ord = 2) > epsilon:
				...
				print "Can't find more counterexamples, no safe policy found"
				return None
		else:
			return None
...
def counterexample(grids, mu, gamma = 0.99, safety = 0.01, epsilon = 1e-5, steps = 64):
	...
	for path in range(len(paths)):
		...
		y = paths[path][state]/grids.x_max
		x = paths[path][state]%grids.x_max
		while gamma**(state - 1)  > epsilon and steps >= state:
...
def output_model(grids, start, policy, steps = None, unsafe = None, safety = 0.01):
	...
	os.system('/home/zekunzhou/workspace/Safety-AI-MDP/mountaincar/prism-4.4.beta-src/src/demos/run')
	file = open('grid_world.pm', 'a')
	file.write("init x = " + str(start[0][1]) + "& y = " + str(start[0][0]) + " endinit")
	file.close()
...
def model_check_prism(cmd = ['/home/zekunzhou/workspace/Safety-AI-MDP/mountaincar/prism-4.4.beta-src/bin/prism', './grid_world.pm', './grid_world.pctl'], timeout_sec = 5.0):
...
def model_check_comics_timer(cmd = ['sh', '/home/zekunzhou/workspace/Safety-AI-MDP/mountaincar/comics-1.0/comics.sh', './grid_world.conf', '--only_model_checking'], timeout_sec = 10.0):
...
def cex_comics_timer(cmd = ['sh', '/home/zekunzhou/workspace/Safety-AI-MDP/mountaincar/comics-1.0/comics.sh', './grid_world.conf'], timeout_sec = 5.0):
  	kill_proc = lambda p: os.system('kill -9 $(pidof comics)')
  	proc = subprocess.Popen(cmd, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
  	timer = Timer(timeout_sec, kill_proc, [proc])
  	try:
