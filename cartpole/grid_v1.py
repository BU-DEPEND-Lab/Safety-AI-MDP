...
##################
		if random is False:
			if feature_states is None:
				...
				self.loc_med_0=[int(self.y_max-1), int(self.y_max-1)]
			else:
				...
				self.loc_med_0=np.array(feature_states[-1])
			...
			self.loc_med_0 = np.random.randint(1, y_max-1, size=2)

				
##################
		self.features=np.zeros([self.y_max, self.x_max, 5]).astype(float)
		...
		self.feature_1=lambda position:  float((position[0] == self.loc_max_0[0]) & (position[1] == self.loc_max_0[1]))
		self.feature_2=lambda position:  float((position[0] == self.loc_max_1[0]) & (position[1] == self.loc_max_1[1]))
		self.feature_3=lambda position:  float((position[0] == self.loc_min_0[0]) & (position[1] == self.loc_min_0[1]))
		self.feature_4=lambda position:  float((position[0] == self.loc_min_1[0]) & (position[1] == self.loc_min_1[1]))
		self.feature_5=lambda position:  float((position[0] == self.loc_med_0[0]) & (position[1] == self.loc_med_0[1]))
		#self.feature_4 = lambda position: 0.0

		for i in range(0, self.y_max):
			for j in range(0, self.x_max):
				self.features[i, j]= [self.feature_1([i, j]), self.feature_2([i, j]), self.feature_3([i, j]), self.feature_4([i, j]), self.feature_5([i, j])]
		...
		file.write("\nx_h_1\n")
		file.write(str(self.loc_max_1[1]))
		file.write("\ny_h_1\n")
		file.write(str(self.loc_max_1[0]))
		file.write("\nx_l_0\n")
