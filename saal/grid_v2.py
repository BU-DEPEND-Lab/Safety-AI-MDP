	...
	def pi_transitions(self, policy, probability = None):
		...
		for y in range(self.y_max):
			for x in range(self.x_max):
				p_sum = 0.0
				P[y, x] = probability * self.transitions[y, x, int(policy[y, x])]
				for a in range(self.actions):
					P[y, x] += (1.0 - probability) * self.transitions[y, x, a] / self.actions
				'''
				for y_ in range(self.y_max):
					for x_ in range(self.x_max):
						p_sum += P[y, x, y_, x_]	
				'''
				p_sum = np.sum(np.reshape(P[y, x], [self.y_max * self.x_max]))
				#if p_sum > 1.0:
				#	print "Error: transition prob sum > 1.0"
					#return None
		'''
		for y in range(self.y_max):
			for x in range(self.x_max):
