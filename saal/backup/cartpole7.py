...
for i in range(grids.y_max):
	for j in range(grids.x_max):
		grids.features[i, j, 0] = math.exp(- float(j/coords[0]))
		grids.features[i, j, 1] = math.exp(- float((i%coords[1])/coords[1]))
		grids.features[i, j, 2] = math.exp(- float(((i/coords[1])%coords[2])/coords[2]))
		grids.features[i, j, 3] = math.exp(- float(((i/coords[1])/coords[2])/coords[3]))
		for k in range(f):
			grids.features[i, j, k + 4] = math.exp(-0.25 * math.sqrt((i - feature_states[k][0])**2 + (j - feature_states[k][1])**2))
	...
	for s in range(S):
		for a in range(A):
			...
			if np.sum(transitions[s, a]) > 0.0:
				transitions[s, a] = np.round(transitions[s, a]/np.linalg.norm(transitions[s, a], ord = 1), 10)
				transitions[s, a] = transitions[s, a] / np.linalg.norm(transitions[s, a], ord = 1)
				for s_ in range(S):
					p = transitions[s, a, s_]
					if p_sum + p >= 1.0:
						p = 1.0 - p_sum
					if p_sum == 1.0:
						p = 0.0
...
if real == 'Y' or real == 'y':
	...
	_, theta, policy, _  = update.expert_synthesize3(grids, demo_mu, agent, starts, steps = steps, epsilon = epsilon, iteration = 30, gamma = gamma, start_theta = theta, MC = False, unsafe =unsafe, safety = safety, exp_policy = exp_policy)
	#grids.w_features(theta)
