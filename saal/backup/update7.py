...
def expert_synthesize(grids, expert, agent, starts, steps, epsilon=1e-6, iteration=100, gamma=0.5, start_theta= None, MC = False, unsafe = None, safety = 0.0001, exp_policy = None):
	...
	while True:
		...
		if flag is not None and abs(K - k) < epsilon:
			if np.linalg.norm(mus[-1] - mus[-2], ord = 2) < epsilon and np.linalg.norm(mu_Bs[-1] - mu_Bs[-2], ord = 2) < epsilon:
				print ">>>>>>>>>>>>>>>Convergeed<<<<<<<<<<<<<<<<"
		...
		if p_B_sum is not None and p_B_sum > safety:
			...
			while p_B_sum < safety:
				if MC is False:
					...
					new_mu = np.sum(np.reshape(exp_u_G, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]) * np.reshape(p_G, [grids.y_max*grids.x_max, 1]) +   np.reshape(exp_u_B, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]) * np.reshape(p_B, [grids.y_max*grids.x_max, 1]), 0) 
				print "Corresponding feature ", new_mu
				if np.linalg.norm(new_mu - mus[-1], ord = 2) > epsilon:
					print "Same k, found different safe policy"
				else:
					print "Can't find different more safe policy"
					break
				mus = mus + [new_mu]	
				if flag is not None and np.dot(new_theta, expert) > np.dot(flag['weight'], expert):
					flag = {'weight': new_theta, 'policy': new_policy, 'value': new_value, 'unsafe': p_B_sum, 'feature': new_mu}
...
def expert_synthesize2(grids, expert, agent, starts, steps, epsilon=1e-6, iteration=100, gamma=0.5, start_theta= None, MC = False, unsafe = None, safety = 0.0001):
	...
	if p_B_sum <= safety and p_B_sum is not None:
		...
		new_mu_B =  counterexample(grids, grids.features, gamma, safety, epsilon, steps)
		mu_Bs = mu_Bs + [new_mu_B]
		print "Add 1st counterexample features ", new_mu_B	

		print ">>>>>>>>>>>Looking for initial safe policy"

		new_theta = np.random.randint(100, size = len(grids.features[-1][-1]))
		new_theta = np.array(new_theta).astype(float)/np.linalg.norm(new_theta, ord=2)
		print "Randomly start with weight", new_theta
		grids.w_features(new_theta)
	...
	while p_B_sum > safety:
		...
		print "Corresponding feature ", new_mu
		if len(mus_) > 0:
			if np.linalg.norm(new_mu - mus_[-1], ord=2) < epsilon:
				print "Can't find more candidate policy"
				print "Failed to find safe policy"
				return None
		mus_ = mus_ + [new_mu]

		print "Generating counterexamples until find a safe candidate" 
		new_mu_B =  counterexample(grids, grids.features, gamma, safety, epsilon, steps)
		#new_mu_B = np.sum(np.reshape(exp_u_B, [grids.y_max*grids.x_max, len(grids.features[-1][-1])]), 0) 
		if new_mu_B is not None:
			if np.linalg.norm(new_mu_B - mu_Bs[-1]) < epsilon:
				print "Can't find different counterexample, looking for safe policy fails"
				print "Failed to find safe policy"
				return None
			print "Add counterexample features ", new_mu_B	
			mu_Bs = mu_Bs + [new_mu_B]
		...
		#new_index, new_theta, w_delta_mu = expert_update_theta(grids, np.zeros(len(grids.features[-1][-1])), agent, steps, policies, mu_Bs, gamma, epsilon)
		_, new_theta, _ = expert_update_theta_v1(grids, agent, steps, policies, mus_, mu_Bs, gamma, epsilon)
		new_theta = np.reshape(new_theta, [len(grids.features[-1][-1])])
		thetas.append(new_theta)
		grids.w_features(new_theta)
		print "Weight learnt from counterexamples ", new_theta
...
def expert_synthesize3(grids, expert, agent, starts, steps, epsilon=1e-6, iteration=100, gamma=0.5, start_theta= None, MC = False, unsafe = None, safety = 0.0001, exp_policy = None):
	...
	while True:
		...
		file = open('log', 'a')
		file.write(">>>>>>>>> " + str(i) + "th iteration, candidate theta: "+ str(new_theta) + "; unsafe probability: " + str(p_B_sum) + "; feature " + str(new_mu) + " = " + str(1.0 - K) + "*safe + " + str(K) + "*expert\n") 
		file.close()
		i = i + 1
		
		if flag is not None and abs(K_CEX - K_EXP) < epsilon:
			if len(mus) > 2 and len(mu_Bs) > 2:
				if np.linalg.norm(mus[-1] - mus[-2], ord = 2) < epsilon or np.linalg.norm(mu_Bs[-1] - mu_Bs[-2], ord = 2) < epsilon:
					print ">>>>>>>>>>>>>>>Convergeed<<<<<<<<<<<<<<<<"
			#print "Feature deviation from expert ", np.linalg.norm(new_mu - expert, ord = 2)
			#print "Expert can get value ", np.dot(theta, expertw)
			#flag = [new_theta, new_policy, new_value, new_mu]
...
def counterexample(grids, mu, gamma = 0.99, safety = 0.01, epsilon = 1e-5, steps = 64):
	...
	while safety_ > safety * epsilon :
		...
		file.close()
		cex_comics_timer(['sh', '/home/zekunzhou/workspace/Safety-AI-MDP/cartpole/comics-1.0/comics.sh', './grid_world.conf'], int(2 * math.log(grids.y_max * grids.x_max / 64))
		try:
...
def output_model(grids, starts, policy, steps = None, unsafe = None, safety = 0.01):
	...
	for i in range(grids.y_max * grids.x_max):
		...
		if p_sum > 1.0:
			...
			transitions_.append(str(i) + ' ' + str(i) + ' ' + str(1.0 - p_sum) + '\n')
			transitions.append(str(i) + ' ' + str(i) + ' ' + str(1.0 - p_sum) + '\n')
	...
	file.write('STATES ' + str(1 + grids.y_max * grids.x_max) + '\n')
	...
	for start in range(0, len(starts)):
		os.system('export DIR=/home/zekunzhou/workspace/Safety-AI-MDP/cartpole')
		os.system('/home/zekunzhou/workspace/Safety-AI-MDP/cartpole/prism-4.4.beta-src/src/demos/run')
...
def model_check_prism(cmd = ['/home/zekunzhou/workspace/Safety-AI-MDP/cartpole/prism-4.4.beta-src/bin/prism', './grid_world.pm', './grid_world.pctl'], timeout_sec = 5.0):
...
def model_check_comics_timer(cmd = ['sh', '/home/zekunzhou/workspace/Safety-AI-MDP/cartpole/comics-1.0/comics.sh', './grid_world.conf', '--only_model_checking'], timeout_sec = 10.0):
...
def cex_comics_timer(cmd = ['sh', '/home/zekunzhou/workspace/Safety-AI-MDP/cartpole/comics-1.0/comics.sh', './grid_world.conf'], timeout_sec = 5.0):
  	kill_proc = lambda p: os.system('kill -9 $(pidof comics)')
  	proc = subprocess.Popen(cmd, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
  	timer = Timer(timeout_sec, kill_proc, [proc])
  	try:
