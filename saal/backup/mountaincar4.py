...
unsafe = []
for s in range(S):
	#if (int(s%coords[0]) <= (coords[0] -1) * 3/18 and int(s/coords[0]) <= (coords[1] -1 ) * 4/14) or (int(s%coords[0]) >= (coords[0] - 1) * 15/18 and int(s/coords[0]) >= (coords[1] - 1) * 10/14) or int(s/coords[0]) >= (coords[1] - 1) * 13/14: 
	if (int(s%coords[0]) <= (coords[0] -1) * 3/18 and int(s/coords[0]) <= (coords[1] -1 ) * 4/14) or (int(s%coords[0]) >= (coords[0] - 1) * 18/18 and int(s/coords[0]) >= (coords[1] - 1) * 14/14) or int(s/coords[0]) >= (coords[1] - 1) * 16/14: 
...
while real == 'Y' or real == 'y':
	...
	try:
		...
		break
	_, theta, policy, p = update.expert_synthesize1(grids, demo_mu, agent, starts, steps = steps, epsilon = epsilon, iteration = iteration, gamma = gamma, start_theta = theta, MC = False, unsafe =unsafe, safety = safety, exp_policy = exp_policy)
	#grids.w_features(theta)
