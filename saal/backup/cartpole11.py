...
unsafe = []
for s in range(S):
	if int(s%coords[0]) <= 0 or int(s%coords[0]) >= (coords[0] - 1 + 2): 
...
while real == 'Y' or real == 'y':
	...
	try:
		...
		break
	_, theta, policy, _  = update.expert_synthesize2(grids, demo_mu, agent, starts, steps = steps, epsilon = epsilon, iteration = iteration, gamma = gamma, start_theta = theta, MC = False, unsafe =unsafe, safety = safety, exp_policy = exp_policy)
	#grids.w_features(theta)
