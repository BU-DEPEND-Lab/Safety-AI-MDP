...
for i in range(f[1]):
	feature_states.append([])
	for j in range(f[0]):
		feature_states[-1].append([int(S/coords[0] * j/(f[0] - 1)), int(coords[0] * i/(f[1] - 1))])
'''
...
'''
for s in range(S):
	if int(s%coords[0]) <= 0  or int(s%coords[0]) >= coords[0]: 
...
policy = MDP.QSoftmaxPolicy(solver.getQFunction());
