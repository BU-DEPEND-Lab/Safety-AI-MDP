...
for i in range(grids.y_max):
	for j in range(grids.x_max):
		grids.features[i, j, 0] = math.exp(- coords[0] * float(1.0 * j/coords[0]))
		grids.features[i, j, 1] = math.exp(- coords[1] * float((1.0 * i%coords[1])/coords[1]))
		grids.features[i, j, 2] = math.exp(- coords[2] * float(((1.0 * i/coords[1])%coords[2])/coords[2]))
		grids.features[i, j, 3] = math.exp(- coords[3] * float(((1.0 * i/coords[1])/coords[2])/coords[3]))
		for k in range(f):
...
if real == 'Y' or real == 'y':
	...
	try:
		...
		pass
	_, theta, policy, _  = update.expert_synthesize2(grids, demo_mu, agent, starts, steps = steps, epsilon = epsilon, iteration = 30, gamma = gamma, start_theta = theta, MC = False, unsafe =unsafe, safety = safety, exp_policy = exp_policy)
	#grids.w_features(theta)
