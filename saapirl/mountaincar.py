...
unsafe = []
for s in range(S):
	if (int(s%coords[0]) <= (coords[0] -1) * 2/18 and int(s/coords[0]) <= (coords[1] - 1) * 2/14) or (int(s%coords[0]) >= (coords[0] - 1) * 15/18 and int(s/coords[0]) >= (coords[1] - 1) * 11/14) or int(s/coords[0]) >= (coords[1] - 1) * 16/14: 
...
def observationToState(o, thresh):
	...
	## if o[i] > thresh[i], then ox = 2*thresh[i]
        	val = int(coords[i] * ox / (thresh[i, 1] - thresh[i, 0]));
...
while real == 'Y' or real == 'y':
	...
	for i_episode in xrange(maxepisodes):
		...
		if add_start:
    		...
    		demo_mu_episode = np.zeros(len(grids.features[-1][-1]))
		demo_mu_episode += grids.features[int(start_s/coords[0])][int(start_s%coords[0])]
			...
			for i in range(combo):
				...
				#transitions[s, a, s1] += 1.0
		    		#demo_mu_episode += grids.features[int(s1/coords[0])][int(s1%coords[0])] * (gamma**((combo + 1) * t + (i + 1)))
				for s_ in unsafe:
			...
			for s_ in unsafe:
				if s_[0] * coords[0] + s_[1] == s1:
					tripping = True
		    	#demo_mu_episode += grids.features[int(s1/coords[0])][int(s1%coords[0])] * (gamma**((combo + 1) * (t + 1)))
		    	demo_mu_episode += grids.features[int(s1/coords[0])][int(s1%coords[0])] * (gamma**(t + 1))
		...
		demo_mu += demo_mu_episode
			...
			if tripping is False:
				...
				for tt in range(t + 1, steps - 1):
		    			demo_mu_episode += grids.features[int(s/coords[0])][int(s%coords[0])] * (gamma**(tt + 1))
				demo_mu += demo_mu_episode
			'''
  		else:
			rew = -200
	...
	file = open('mountaincar_unsafe', 'w')
	for b in unsafe:
		file.write(str(b[0]) + ':' + str(b[1]) + '\n')
	file.close()

	file = open('mountaincar_expert_policy', 'w')
...
real = raw_input("Try AI Toolbox policy? [Y/N]")	
while real == 'y' or real == 'Y':
	file = open('mountaincar_expert_policy', 'r')
	...
	while real == 'Y' or real == 'y':
		...
		for t in xrange(steps):
			...
			for s_ in unsafe:
	    		...
	    		env.render()
		print "Finished after {}, tripping {}".format(t+1, tripping)
...
#	cProfile.runctx('test_expert_train(grids, demo_mu, agent, starts, epsilon, steps, iteration, gamma, start_theta, MC, safety, unsafe)', {'grids': grids, 'demo_mu': demo_mu, 'agent': agent, 'epsilon': 1e-5, 'starts': starts, 'steps': steps, 'iteration': 30, 'gamma': gamma, 'start_theta': None, 'MC': False, 'safety': None, 'unsafe': unsafe, 'toolbox': True, 'test_expert_train': update.expert_train}, {})
	...
	file = open('mountaincar_policy', 'w')
	for i in range(len(demo_mu)):
		file.write(str(demo_mu[i])+':')
	file.write('\n')
	for i in range(len(theta)):
...
real = raw_input("Apprenticeship Learning from AI Toolbox Policy? [Y/N]")
if real == 'y' or real == 'Y':
	file = open('mountaincar_MDP', 'r')
	lines = file.readlines()
	if lines[0] == 'starts' or lines[0] == 'starts\n':
	...
	expert_policy = np.zeros([grids.y_max, grids.x_max]).astype(float)
	for i in range(2, len(lines)):
		line = re.split(':|\n|''', lines[i])
		j = 0
		for k in range(len(line)):
			if line[k] != '':
				expert_policy[i-2, j] = float(line[k])
				j += 1
	file.close()
	demo_mu, _ = update.optimal_feature(grids, starts, steps, expert_policy, epsilon = epsilon, gamma=gamma)
	print "Second hand feature ", demo_mu
	_, theta, policy ,_ = update.expert_train(grids, demo_mu, agent, epsilon = epsilon, starts = starts, steps = steps, iteration = 30, gamma= gamma, start_theta = np.array(demo_mu)/np.linalg.norm(demo_mu, ord=2), MC = False, safety = None, unsafe = unsafe)
	grids.w_features(theta)
	demo_mu, _ = update.optimal_feature(grids, starts, steps, policy, epsilon = epsilon, gamma=gamma)
...
real = raw_input("Try policy? [Y/N]")	
while real == 'y' or real == 'Y':
	file = open('mountaincar_policy', 'r')
	lines = file.readlines()
...
while real == 'Y' or real == 'y':
	...
	for i in al_mu_str:
		if i != '' and i!=']' and i!= ' ' and i!='\n':
			al_mu.append(float(i))
	al_mu = np.array(al_mu)
	print al_mu
	...
	#unsafe = []
	#file = open('mountaincar_unsafe', 'r')
	#lines = file.readlines()
	...
	print "AL model checking unsafe prob ", p_B_sum
	'''
	...
	safety_spec = re.split(':|,|' '', safety_spec)
	for safety in safety_spec:
		try:
			safety = float(safety)
		except:	
			break
		_, theta, policy, p = update.expert_synthesize(grids, al_mu, agent, starts, steps = steps, epsilon = epsilon, iteration = iteration, gamma = gamma, start_theta = theta, MC = False, unsafe =unsafe, safety = safety, exp_policy = None)
		#grids.w_features(theta)
		#learnt_policy, value = update.optimal_value(grids, agent, steps = steps, epsilon= 1e-5, gamma = gamma, toolbox = False, starts = starts)
		file_name = "mountaincar_policy_safe" + str(safety)
		file = open(file_name, 'w')
		#file = open('mountaincar_policy_safe', 'w')
...
while real == 'Y' or real == 'y':
	file = open('mountaincar_policy_safe', 'r')
	lines = file.readlines()
...
while real == 'Y' or real == 'y':
	...
	print "policy_safe {} average socre {}, tripping times {}, rate {}".format(spec, average_, tripping_rate_, tripping_rate_/maxepisodes)
	file = open('log', 'a')
	file.write('\n\n')
	file.write("AL policy average socre {}, tripping times {}, rate {}".format(average, tripping_rate, tripping_rate/maxepisodes))
	file.write('\n')
	file.write("policy_safe {} average socre {}, tripping times {}, rate {}".format(spec, average_, tripping_rate_, tripping_rate_/maxepisodes))
	file.write('\n\n')
	file.close()
	real = raw_input("Compare policies? [Y/N]")
